      ...
      GET: async ({ request }) => {
        ...
        try {
          ...
          if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
            console.error("[MP OAuth] SUPABASE_SERVICE_ROLE_KEY missing in server environment");
          ...
          try {
            tokenJson = JSON.parse(rawBody) as Record<string, unknown>;
          } catch {
            // Mercado Pago should return JSON, but keep the redirect safe if it doesn't.
          }

          if (!tokenResp.ok || !tokenJson.access_token) {
            console.error("[MP OAuth] token exchange failed", tokenResp.status, rawBody);
            console.error("[MP OAuth] exchange context", JSON.stringify({
            ...
            return redirect(`/pix?mp=error&reason=token_exchange&detail=${encodeURIComponent(String(mpMsg))}`);
          }
          ...
          console.error("[MP OAuth] unexpected callback failure", message);
          const detail = message.includes("SUPABASE_SERVICE_ROLE_KEY") ? "SUPABASE_SERVICE_ROLE_KEY" : "unexpected";
          return redirect(`/pix?mp=error&reason=server_config&detail=${encodeURIComponent(detail)}`);
        }
      },
    },
  },
